...
def test_caltech101(batch_size):
    ...
    targets = getattr(dm.data_train, "dataset", dm.data_train).targets
    assert min(set(targets)) == 0
    assert max(set(targets)) == len(dm.classes) - 1
...
def test_imagenet(batch_size):
    ...
    targets = getattr(dm.data_test, "dataset", dm.data_test).targets
    assert min(set(targets)) == 0
    assert max(set(targets)) == len(dm.classes) - 1
